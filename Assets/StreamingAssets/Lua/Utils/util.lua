...
-- node: 监听对象
-- fun: 监听回调
local function addListener(object, node, fun)
	if object.__listenerList == nil then
		object.__listenerList = {}
	end

	_fun = function(...)
		fun(object, ...)
	end
	object.__listenerList[node] = _fun
	
	node:AddListener(_fun)
end

-- description: 清除监听事件
-- object: lua对象
local function clearListener(object)
	if object.__listenerList == nil then
		return
	end

	for node, fun in pairs(object.__listenerList) do
		node:RemoveListener(fun)
		--node:Invoke()
	end
end
...
-- object: lua对象
-- node: 监听对象
local function removeListener(object, node)
    if object.__listenerList == nil then
        return
    end

    node:RemoveListener(object.__listenerList[node] )
end
